...
\subsection{S8.2 Strong coupling from crossing number}
...
Physical interpretation: the linking enhancement goes as $(\text{crossing number})^2 \approx p^4$ for the torus knot family.
...
\begin{itemize}
  \item $\Lambda_{\text{tube}} = \hbar c/(\alpha R_p)$, where $R_p$ is the self-consistent proton torus radius
...
\begin{itemize}
  \item $\Lambda_{\text{tube}}$ --- derived from $\alpha$ and $R_p$
...
\textbf{Torus knot (this work)} & \textbf{23} & \textbf{3 + 3 integers} & \textbf{Yes} \\
\bottomrule
\end{tabular}
