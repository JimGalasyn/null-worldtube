...
Decomposition:
\begin{itemize}
  \item $2\pi/3$: $\mathbb{Z}_3$ generation symmetry from Borromean link
  \item $2/9 = pq/N_c^2$: toroidal--poloidal coupling through $3\times 3$ interaction matrix
\end{itemize}
...
Physical interpretation: Borromean linking reduces the Koide angle. As linking increases ($|q| \to 1$), the angle approaches $2\pi/3$.
...
\begin{itemize}
  ...
  \item Denominator: $21 = N_c(2N_c+1) = 3\times 7$ (color $\times$ extended group dimension)
\end{itemize}

This is one of the most precise predictions in the model (6~ppm). The factor $2N_c+1 = 7$ corresponds to the dimension of the fundamental representation of $\mathrm{SO}(2N_c+1) = \mathrm{SO}(7)$; its appearance in the lepton anchor mass warrants further investigation.
...
The charm-to-top ratio IS the fine-structure constant. The strange-to-bottom ratio is $\alpha \times N_c$.
...
Numerator: knot metric $p^2+q^2 = 5$. Denominator: $N_c^2 = 9$.
...
\subsection{S12.1 Four inputs}

The model requires four measured inputs:
\begin{enumerate}
  \item $\alpha = 1/137.036$ --- the fine-structure constant (claimed to emerge as $r/R$, but used numerically as input)
  \item $m_e = 0.51100$~MeV --- the electron mass (sets the fundamental torus radius)
  \item $m_\mu = 105.658$~MeV --- the muon mass (sets the lepton Koide scale $S$)
  \item $\Lambda_{\text{tube}} = \hbar c/(\alpha R_p) = 255.7$~GeV --- the tube energy scale, set by the proton torus radius $R_p$. Since $R_p$ depends on the proton mass $m_p$, which is not derived within this framework, $\Lambda_{\text{tube}}$ is an independent input. Deriving $\Lambda_{\text{tube}}$ from $(\alpha, m_e, m_\mu)$ alone --- i.e., reducing the input count to three --- requires a first-principles calculation of the proton mass from the torus knot topology. This is an open problem and a priority for future work.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $N_c = 3$ --- Borromean link number (baryon topology)
\end{enumerate}
...
\begin{itemize}
  \item $\theta_K = (6\pi+2)/9$ from $(p,q,N_c)$
  \item $\theta(q_{\text{em}}) = (6\pi + 2/(1+3|q|))/9$ from charge
  \item All coefficients: $2/3$, $\sqrt{5}$, $20/21$, $3/13$, $16$, $2/9$, $\pi-2$, etc.\ --- pure functions of $(p,q,N_c)$
\end{itemize}
...
\textbf{Torus knot (this work)} & \textbf{23} & \textbf{4 + 3 integers} & \textbf{Yes} \\
\bottomrule
\end{tabular}
