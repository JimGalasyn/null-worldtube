...
From $v$ and $\sin^2\theta_W$ via the Weinberg--Salam mass relations:
\begin{align}
  m_W &= \frac{v}{2}\,\frac{e(m_Z)}{\sin\theta_W}, \label{eq:mW}\\[4pt]
  m_Z &= \frac{m_W}{\cos\theta_W}, \label{eq:mZ}
\end{align}
where $e(m_Z) = \sqrt{4\pi\alpha(m_Z)}$ is the running electromagnetic coupling at the $Z$ scale ($\alpha(m_Z)^{-1} \approx 127.9$). Substituting the NWT predictions $v = 246.34$~GeV and $\sin^2\theta_W = 3/13$:
\begin{align}
  m_W &= 80.4~\text{GeV} \quad (\text{PDG: } 80.37,\ 0.0\%), \\
  m_Z &= 91.6~\text{GeV} \quad (\text{PDG: } 91.19,\ 0.5\%).
\end{align}
...
\begin{itemize}
  \item \textbf{$\alpha$}: Skilton's formula $\alpha^{-1} = \sqrt{137^2 + \pi^2} = 137.036016$ (0.12~ppm), where $137 = p_s^2 + q_s^2$ with generators $p_s = 2k + p^2 + q^2 = 11$ and $q_s = p^2 = 4$ --- these are NWT quantum numbers. See Paper~2~\cite{galasyn2026b}.
  \item \textbf{$m_\mu$}: The Koide angle $\theta_K = (6\pi+2)/9$ is a function of $(p,q,k)$, so given $m_e$, the Koide relation determines $m_\mu = 105.658$~MeV (0.001\% error). See Paper~2~\cite{galasyn2026b}.
  \item \textbf{$\Lambda_{\text{tube}}$}: The Lenz formula $m_p/m_e = 6\pi^5$ (0.002\% error) is structurally explained as $2k \times \pi^{p^2+q^2}$. Given $m_e$, this yields $m_p \to R_p \to \Lambda_{\text{tube}}$. See Paper~2~\cite{galasyn2026b}.
\end{itemize}
...
See Paper~2~\cite{galasyn2026b} for the complete derivation of steps 1--3.
...
All predictions are reproducible from the open-source simulation code, available at \url{https://github.com/JimGalasyn/null-worldtube}:
\begin{verbatim}
