...
A skeptical reader may ask: was the formula
$1/\alpha = q_e^2 \times \Phi_\text{geom} + 1$ derived
\emph{before} comparing to $137$, or was it \emph{found} by
searching for combinations that match?  The answer is the former.
The formula's structure is dictated by the Bogomolny identity
(\S\ref{sec:bps}):
\begin{itemize}\setlength\itemsep{0pt}
  \item $q_e^2$ is the eigenvalue of the Laplacian
    $-\partial^2/\partial\varphi^2$ on the toroidal mode
    $e^{iq\varphi}$.  This is the \emph{only} operator and
    eigenfunction compatible with the periodicity of the torus
    knot.
  \item $\Phi_\text{geom}$ is the total $D_q$ geometric phase,
    determined by the knot symmetry and the BPS flux quantum.
    Its value $\pi\sqrt{3}$ is fixed once the trefoil is chosen.
  \item The additive $+1$ is the topological winding number,
    which enters as a boundary term in the Bogomolny identity.
    Its integrality is forced by gauge topology.
\end{itemize}
The multiplicative--additive structure
$1/\alpha = (\text{eigenvalue}) \times (\text{phase}) + (\text{winding})$
is the \emph{only} decomposition consistent with the Bogomolny
identity on a $D_q$-symmetric flux tube.  It was not selected
from a menu of candidate formulas.
...
the gauge-group structure ($\su(3)$), while the cinquefoil
$T(2,5)$ provides the grand-unified embedding ($\su(5) \supset
\su(3) \times \su(2) \times \uu(1)$).  The coupling $\alpha$ is
determined by the interplay between these two levels of the knot
hierarchy---the SM lives inside the GUT, and $\alpha$ measures
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item The cinquefoil eigenvalue $q^2 = 25$
    (\S\ref{sec:why-cinquefoil}).
\end{itemize}
...
\begin{enumerate}\setlength\itemsep{2pt}
  ...
  \item Knot selection is forced by minimality: the trefoil
    $T(2,3)$ is the simplest non-trivial torus knot, and the
    cinquefoil $T(2,5)$ is the unique next member whose eigenvalue
    produces $1/\alpha \approx 137$.
  ...
  \item The seven-step Aharonov--Bohm holonomy assembles these
    ...
    factor traced to a prior step and confirmed by numerical
