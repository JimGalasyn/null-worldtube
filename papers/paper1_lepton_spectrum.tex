...
We present such a framework. A single geometric object --- a photon circulating on a $(2,1)$ torus knot --- combined with three topological integers ($p=2$, $q=1$, $N_c=3$) and four measured inputs ($\alpha$, $m_e$, $m_\mu$, $\Lambda_{\text{tube}}$), determines all 23 remaining Standard Model parameters. The median prediction error is 0.7\%.
...
\begin{table}[h]
\centering
\caption{All 23 predictions from 4 inputs ($\alpha$, $m_e$, $m_\mu$, $\Lambda_{\text{tube}}$) and 3 integers ($p=2$, $q=1$, $N_c=3$).}
\label{tab:scorecard}
\small
\begin{tabular}{@{}clllc@{}}
\toprule
